...
void    animation_caracter(t_game *game, int x, int y)
{
    ...
    // void *xpm_img_c_rr = mlx_xpm_file_to_image(game->mlx, game->img.x_p_c_rr, &game->x_w, &game->x_h);
    // void *xpm_i_c_u = mlx_xpm_file_to_image(game->mlx, game->img.x_p_c_u, &game->x_w, &game->x_h);
    // void *xpm_img_c_uu = mlx_xpm_file_to_image(game->mlx, game->img.x_p_c_uu, &game->x_w, &game->x_h);
    // void *xpm_img_c_l = mlx_xpm_file_to_image(game->mlx,  game->img.x_p_c_l, &game->x_w, &game->x_h);
    // void *xpm_img_c_ll = mlx_xpm_file_to_image(game->mlx,  game->img.x_p_c_ll, &game->x_w, &game->x_h);
    // void *xpm_img_c_d = mlx_xpm_file_to_image(game->mlx,  game->img.x_p_c_d, &game->x_w, &game->x_h);
    // void *xpm_img_c_dd = mlx_xpm_file_to_image(game->mlx,  game->img.x_p_c_dd, &game->x_w, &game->x_h);

    // right
    if ((game->player_sprite == 0) && (game->steps % 2 == 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c, x * game->x_w, y * game->x_h);
    else if ((game->player_sprite == 0) && (game->steps % 2 != 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c_rr, x * game->x_w, y * game->x_h);
    
    // up
    if ((game->player_sprite == 1) && (game->steps % 2 == 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c_u, x * game->x_w, y * game->x_h);
    else if ((game->player_sprite == 1) && (game->steps % 2 != 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c_uu, x * game->x_w, y * game->x_h);

    // left
    else if ((game->player_sprite == 2) && (game->steps % 2 == 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c_l, x * game->x_w, y * game->x_h);
    else if ((game->player_sprite == 2) && (game->steps % 2 != 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c_ll, x * game->x_w, y * game->x_h);

    // down
    else if ((game->player_sprite == 3) && (game->steps % 2 == 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c_d, x * game->x_w, y * game->x_h);
    else if ((game->player_sprite == 3) && (game->steps % 2 != 0))
        mlx_put_image_to_window(game->mlx, game->win, game->img.x_i_c_dd, x * game->x_w, y * game->x_h);
